...
public class L699_FallingSquares {
  public static void main(String[] args) {
       Solution solution = new L699_FallingSquares().new Solution();
      System.out.println(solution.fallingSquares(new int[][]{{1,2},{2,3},{6,1}}));
  }
class Solution {
    public List<Integer> fallingSquares(int[][] positions) {
        int[] hs = new int[positions.length];
        for (int i = 0; i < positions.length; i++) {
            ...
            for (int j = i+1; j < positions.length; j++) {
                ...
                int right2 = left2 + height2;
                if (left2 < right && left < right2) { //有交集
                    hs[j] = Math.max(hs[j], hs[i]);
                }
            }
        }
